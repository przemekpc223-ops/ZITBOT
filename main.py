...
@bot.event
async def on_ready():
    print(f'ZITBOT online! Wszystko ustawione pod dwie wiadomości.')
    check_time_loop.start()

# --- 1. ALERT (Dwie osobne wiadomości) ---
@bot.command()
@commands.has_permissions(administrator=True)
async def alert(ctx, *, message):
    await ctx.message.delete()
    # Pierwsza wiadomość: sam obrazek
    await ctx.send(IMG_URL)
    # Druga wiadomość: sam tekst
...
# --- 3. AUTOWIADOMOŚĆ (Dwie osobne wiadomości) ---
@bot.command()
@commands.has_permissions(administrator=True)
async def autowiad(ctx, czas: str, *, tresc: str):
    try:
        ...
        await ctx.send(f"✅ Ustawiono! O **{czas}** wyślę obrazek i tekst osobno.")
    except:
        await ctx.send("❌ Format: `!autowiad 13:00 treść`.")
